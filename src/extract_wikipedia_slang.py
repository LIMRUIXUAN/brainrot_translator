...
DEFAULT_HTML_INPUT = Path("data/raw/wikipedia_glossary.html")
...
def clean_text(value: str) -> str:
    ...
    value = re.sub(
        r"\[\s*(?:\d+|[a-z]|citation needed|unreliable source\?|better source needed)\s*\]",
        "",
        value,
        flags=re.I,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input",
        default=str(DEFAULT_HTML_INPUT),
        help="Path to the local Wikipedia HTML source file.",
    )
